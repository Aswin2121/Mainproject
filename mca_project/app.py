...
# Test email function
def test_email_connection(email):
    """Test if the email address can receive emails by sending a test message"""
    try:
        msg = MIMEMultipart()
        msg['From'] = SENDER_EMAIL
        msg['To'] = email
        msg['Subject'] = "Flood Prediction System - Email Verification"
        
        body = """
        This is a test email to verify your email address for the Flood Prediction System.
        
        If you received this email, your address is valid and you'll receive flood alerts here.
...
def register_user(username, password, email, locality):
    ...
    with st.spinner("Verifying email address..."):
        if not test_email_connection(email):
            return False, "Email verification failed. Please check your email address and try again."
...
def save_alert(username, alert_message, risk_level, details=None):
    ...
    # Send email to ALL registered users for every prediction
    users = get_users()
    for user, user_data in users.items():
        if "email" in user_data.get("alert_channels", []) and user_data.get("email_verified", False):
            send_email_alert(
                user_data["email"],
                alert_message,
                risk_level,
...
def display_location_on_map(df, selected_lat=None, selected_lon=None):
    st.subheader("Interactive Location Map")
    
    # Prepare the data for the map
    map_data = df[['Latitude', 'Longitude']].copy()
    map_data['color'] = '[0, 0, 255, 200]'  # Blue for all points
    map_data['size'] = 1000  # Base size for all points
    
    # Highlight selected location if provided
    if selected_lat is not None and selected_lon is not None:
        # Add selected location with different color
        selected_data = pd.DataFrame({
            'Latitude': [selected_lat],
            'Longitude': [selected_lon],
            'color': ['[255, 0, 0, 200]'],  # Red for selected
            'size': [1500]  # Larger size for selected
        })
        map_data = pd.concat([map_data, selected_data])
    ...
    # Create the tooltip
    tooltip = {
        "html": "<b>Latitude:</b> {Latitude:.4f}<br/><b>Longitude:</b> {Longitude:.4f}",
        "style": {
    ...
    if r.last_clicked:
        st.session_state.selected_location = {
            'latitude': r.last_clicked['coordinates'][1],
            'longitude': r.last_clicked['coordinates'][0]
        }
        st.success(f"Selected location: Latitude {r.last_clicked['coordinates'][1]:.4f}, Longitude {r.last_clicked['coordinates'][0]:.4f}")


# Function to get place name from latitude and longitude
def get_place_name(latitude, longitude):
    geolocator = Nominatim(user_agent="flood_prediction_system", timeout=10)
...
#             else:
#                 success, message = register_user(new_username, new_password,email, locality)
#                 if success:
#                     st.success(message)
#                 else:
#                     st.error(message)
...
def user_settings():
    ...
    with col1:
        ...
        if user_data.get("email_verified", False):
            ...
            if st.button("Verify Email"):
                if test_email_connection(user_data["email"]):
                    users[st.session_state.username]["email_verified"] = True
                    save_users(users)
                    st.success("Email verified successfully!")
                else:
                    st.error("Failed to verify email. Please check your email address.")
    ...
    if st.button("Save Settings"):
        ...
        if email != user_data["email"]:
            ...
            with st.spinner("Verifying new email address..."):
                if not test_email_connection(email):
                    st.error("Email verification failed. Please check your email address.")
                    return
    ...
    if not st.session_state.logged_in:
        ...
        if page == "Data Overview":
            ...
            if not models or scaler is None:
                ...
                col1, col2 = st.columns(2)
                with col1:
                    selection_method = st.radio("Selection Method", ["Map Click", "Dropdown"])
                ...
                if selection_method == "Dropdown":
                    ...
                    if st.session_state.selected_location:
                        selected_lat = st.session_state.selected_location['latitude']
                        selected_lon = st.session_state.selected_location['longitude']
                        
                        # Find the closest point in our dataset
                        distances = np.sqrt((df['Latitude'] - selected_lat)**2 + (df['Longitude'] - selected_lon)**2)
                        closest_idx = distances.idxmin()
                        selected_row = df.loc[[closest_idx]]
                        selected_lat = selected_row['Latitude'].values[0]
                        selected_lon = selected_row['Longitude'].values[0]
                        
                        st.info(f"Selected location: Latitude {selected_lat:.4f}, Longitude {selected_lon:.4f}")
                
                # Display the map with all locations and selected location highlighted
                display_location_on_map(df, selected_lat, selected_lon)
                
                if selected_lat is None or selected_lon is None:
                    st.warning("Please select a location using either the map or dropdown")
                    return
                ...
                st.subheader("Selected Location Data")
                ...
                if st.button("Predict"):
                    prediction, probability = predict_flood(models[selected_model], selected_row.drop(columns=['Latitude', 'Longitude', 'Flood Occurred']), scaler)
                    ...
                    model_predictions = {}
                    for name, model in models.items():
                        pred, prob = predict_flood(model, selected_row.drop(columns=['Latitude', 'Longitude', 'Flood Occurred']), scaler)
...
def login_page():
    ...
    with tab2:
        ...
        new_username = st.text_input("Username", help="Choose a unique username", key="reg_username")
        new_password = st.text_input("Password", type="password", 
                                   help="At least 6 characters", key="reg_password")
        confirm_password = st.text_input("Confirm Password", type="password", key="confirm_password")
        email = st.text_input("Email Address", 
                            help="Must be a valid email address (e.g., user@example.com)", 
                            key="reg_email")
        locality = st.text_input("Locality", help="Your city or region", key="reg_locality")
        ...
        if st.button("Register"):
            ...
            if not new_username:
                ...
                st.error("Email is required or use correct format")
            elif not is_valid_email(email):
                st.error("Invalid email format. Please use format like: user@example.com")
            elif not locality:
